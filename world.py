...
# Définition de varibles
floor = []
coo = [0, 0]
level = 0
pos_p = None

# Fonction qui permet la création d'un niveau
def create_floor(_level):
    global floor
    _level = 0
    if _level == 0: # Le premier niveau est crée dans cette condition
    ...
    if _level == 1: # Le deuxième niveau est crée dans cette condition
        ...
        pos_boss = [9, random.randint(0, 9)]
    if _level == 2: # Le troisième niveau est crée dans cette condition
        ...
        pos_boss = [9, random.randint(0, 9)]
    if _level == 3: # Le quatrième niveau est crée dans cette condition
        ...
        pos_boss = [9, random.randint(0, 9)]
    if _level == 4: # Le dernier niveau est crée dans cette condition
        first_room = room.room_5_0
        liste_level = [room.room_boss_5]
        pos_boss = None
    
    1240
    # Ajoute les salles dans le niveau
    for i in range(10):
        floor.append([])    
        for j in range(0, 10):
            if j == 0 and i == 0: 
                floor[i].append(first_room) # Ajoute la première salle
            elif pos_boss != None and i == pos_boss[0] and j == pos_boss[1]:
                floor[i].append(boss_room) # Ajoute la salle de boss
            else:
                floor[i].append(random.choice(liste_level)) # Ajoute les salles de la liste dans le niveau avec des positions aléatoires
...
create_floor(0) # Crée le niveau 0 (premier niveau)

# Fonction qui permet de changer de salle après avoir pris une porte
...
# Fonction qui charge la salle de la porte emprunté
def show_next_room():
    try:
