...
def boucle():
    ...
    for event in pygame.event.get():  # Boucle pour chaque event
        ...
        if event.type == pygame.KEYDOWN:  # S'occupe des évènements quand un bouton est préssé
            ...
            if event.key == pygame.K_u:  # Permet au joueur d'attaquer quand le bouton u est pressé
                ...
                folie += weapon.weapon[weapon.current_weapon][
                    4]  # Augmente la folie du joueur en fonction de l'arme équippée
                if folie >= floie_max:  # Si la folie est supérieur à celle pouvant être supporté, le joueur meurt
                    death()
            if event.key == pygame.K_i:  # Permet au joueur d'utiliser le dash
    ...
    if possible_object_picture == None:  # Si pas d'objet
        rect = pygame.rect.Rect(100, 0, 100,
                                100)  # Créer un rectangle avec ces paramètres (de largeur et hauteur 100 pixel en haut à gauche) et l'associe à la variable
        graphic_main.update.append(
            pygame.draw.rect(graphic_main.screen, (0, 0, 0), rect))  # Affiche le rectangle précédemment défini
    else:  # Si le joueur à un objet
        graphic_main.update.append(
            graphic_main.screen.blit(possible_object_picture, (100, 0)))  # Affiche l'objet avec ces coordonées
    ...
    cooldown -= 1
    cooldown_move -= 1
    # change les images si le joueur est blessé
...
def collision_with_door(door):
    ...
    if rectA.collidelist(door) != -1:
        ...
        if not ennemy.enemy_1_list and not ennemy.enemy_1_2_list and not ennemy.enemy_2_1_list and not ennemy.enemy_2_2_list and not ennemy.boss_list and not ennemy.enemy_slime_list and not ennemy.enemy_4_1_list and not ennemy.enemy_4_2_list and not ennemy.boss_list_2 and not ennemy.boss_list_3 and not ennemy.enemy_3_2_list and not ennemy.enemy_3_2_list and not ennemy.final_boss and timer < 0:
            ...
            if rectA.collidelist(door) == 1:  # Porte du bas
                world.coo[1] -= 1
                world.next_room(3)

            if rectA.collidelist(door) == 2:  # Porte de droite
                world.coo[0] -= 1
                world.next_room(0)

            if rectA.collidelist(door) == 3:  # Porte du haut
                world.coo[1] += 1
                world.next_room(1)
...
def collision_with_chest():
    rectA = graphic_main.frame[graphic_main.current].get_rect(center=(pos[0] + 15, pos[1] + 25))
    # vérifie si le joueur touche un coffre
    if rectA.collidelist(list(graphic_main.chest.values())) != -1:
        # si c'est le cas active la fonction associée au coffre
        rect = list(graphic_main.chest.values())[rectA.collidelist(list(graphic_main.chest.values()))]
        # si le joueur change d'arme remet l'arme qu'il possédait dans le coffre
        ...
        # change le coffre pour l'afficher ouvert
...
# s'occupe de la collision contre le mur pour éviter que le joueur ne les traverse
def collision_with_wall():
    global pos
    rectA = graphic_main.frame[graphic_main.current].get_rect(center=(pos[0] + 15, pos[1] + 25))
    # si le joueur est trops à gauche le déplace de 20 pixels vers la droite
...
# toute les fonctions suivante vérifie si un ennemi touche le joueur
...
# permet au joueur de se soigné contre du mana lorsque le joueur appui sur a
def _heal():
    global heal_duration
    global mana
    global pv
    heal_duration -= 1
    mana -= 20 / 120
    # vérifie que le joueur possède du mana
    if mana < 0:
        mana = 0
        heal_duration = heal_duration_max
    # et qu'il en a assez consommé
    if heal_duration == 0 and mana >= 0:
        pv += heal
        heal_duration = heal_duration_max

# fonction qui s'occupe du lvl up
def lvl_up():
    global xp
    global pv
    global mana_max
...
# permet d'afficher les pvs
def showpv():
    global pv
    w, h = pygame.display.get_surface().get_size()
    widht = 0.3
    height = 0.05
    # crée un rectangle rouge et un rectangle noir
...
def showfolie():
    ...
    rectB = pygame.Rect((w / 300,
                         (h * 20 / 100) + 55 * h / 1080 + (folie / floie_max) * (634 * h / 1080 - 55 * h / 1080),
                         73 * w / 1920, (634 - 55) * h / 1080 - (folie / floie_max) * (634 - 55) * h / 1080 + 1))
